  subroutine addref_mesh_type(object)
    ...
    if (associated(object%refcount)) then
       ...
       id = id + 1
       object%refcount=>new_refcount("mesh_type", object%name)
       object%refcount%id = id
    end if
    
  end subroutine addref_mesh_type
  
  subroutine incref_mesh_type(object)
...
!       FLAbort ("Attempt to incref mesh_type "//trim(object%name)//" which has no references")	! ToDo
    end if
  ...
  end subroutine incref_mesh_type  
  
  subroutine decref_mesh_type(object)
...
!          FLAbort("that should never happen.")		! ToDo
  ...
  end subroutine decref_mesh_type
  ...
  end function has_references_mesh_type
