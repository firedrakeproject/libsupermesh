  subroutine addref_scalar_field(object)
    ...
    type(scalar_field), intent(inout), target :: object
    integer, save :: id = 0
  ...
  subroutine incref_scalar_field(object)
    ...
    !!< then error.
    use libsupermesh_parallel_tools, only: abort_if_in_parallel_region
    type(scalar_field), intent(in), target :: object
...
!       FLAbort ("Attempt to incref scalar_field "//trim(object%name)//" which has no references")	! ToDo
    end if
  ...
  subroutine decref_scalar_field(object)
    ...
    use libsupermesh_parallel_tools, only: abort_if_in_parallel_region
    type(scalar_field), intent(inout) :: object
...
!          FLAbort("that should never happen.")		! ToDo
  ...
  pure function has_references_scalar_field(object) result (has_references)
    ...
    type(scalar_field), intent(in) :: object
