  subroutine addref_scalar_field(object)
    ...
    type(scalar_field_lib), intent(inout), target :: object
    integer, save :: id = 0
    ...
    if (associated(object%refcount)) then
       ...
       id = id + 1
       object%refcount=>new_refcount("scalar_field", object%name)
       object%refcount%id = id
    end if
    
  end subroutine addref_scalar_field
  
  subroutine incref_scalar_field(object)
    !!< Increment the reference count of object. If there are no references
    !!< then error.
    use libsupermesh_parallel_tools, only: abort_if_in_parallel_region
    type(scalar_field_lib), intent(in), target :: object
    ...
    if (.not.associated(object%refcount)) then
       FLAbort ("Attempt to incref scalar_field "//trim(object%name)//" which has no references")
    end if
  ...
  end subroutine incref_scalar_field  
  
  subroutine decref_scalar_field(object)
    ...
    use libsupermesh_parallel_tools, only: abort_if_in_parallel_region
    type(scalar_field_lib), intent(inout) :: object
  ...
  end subroutine decref_scalar_field

  pure function has_references_scalar_field(object) result (has_references)
    !!< Return true if there are any references to object
    type(scalar_field_lib), intent(in) :: object
    logical :: has_references
    
    has_references=associated(object%refcount)

  end function has_references_scalar_field
