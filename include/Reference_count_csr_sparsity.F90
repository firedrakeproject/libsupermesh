  subroutine addref_csr_sparsity(object)
    ...
    type(csr_sparsity), intent(inout), target :: object
    integer, save :: id = 0
    ...
    if (associated(object%refcount)) then
       ...
       id = id + 1
       object%refcount=>new_refcount("csr_sparsity", object%name)
       object%refcount%id = id
    end if
    
  end subroutine addref_csr_sparsity
  
  subroutine incref_csr_sparsity(object)
    !!< Increment the reference count of object. If there are no references
    !!< then error.
    use libsupermesh_parallel_tools, only: abort_if_in_parallel_region
    type(csr_sparsity), intent(in), target :: object
...
!       FLAbort ("Attempt to incref csr_sparsity "//trim(object%name)//" which has no references")	! ToDo
    end if
  ...
  end subroutine incref_csr_sparsity  
  
  subroutine decref_csr_sparsity(object)
    ...
    use libsupermesh_parallel_tools, only: abort_if_in_parallel_region
    type(csr_sparsity), intent(inout) :: object
...
!          FLAbort("that should never happen.")			! ToDo
  ...
  end subroutine decref_csr_sparsity

  pure function has_references_csr_sparsity(object) result (has_references)
    !!< Return true if there are any references to object
    type(csr_sparsity), intent(in) :: object
    logical :: has_references
    
    has_references=associated(object%refcount)

  end function has_references_csr_sparsity
