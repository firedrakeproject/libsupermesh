  subroutine addref_tensor_field(object)
    ...
    type(tensor_field), intent(inout), target :: object
    integer, save :: id = 0
  ...
  subroutine incref_tensor_field(object)
    ...
    !!< then error.
    use libsupermesh_parallel_tools, only: abort_if_in_parallel_region
    type(tensor_field), intent(in), target :: object
...
!       FLAbort ("Attempt to incref tensor_field "//trim(object%name)//" which has no references")	! ToDo
    end if
  ...
  subroutine decref_tensor_field(object)
    ...
    use libsupermesh_parallel_tools, only: abort_if_in_parallel_region
    type(tensor_field), intent(inout) :: object
...
!          FLAbort("that should never happen.")				! ToDo
  ...
  pure function has_references_tensor_field(object) result (has_references)
    ...
    type(tensor_field), intent(in) :: object
