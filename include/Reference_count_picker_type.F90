  subroutine addref_picker_type(object)
    ...
    type(picker_type), intent(inout), target :: object
    integer, save :: id = 0
    ...
    if (associated(object%refcount)) then
       ...
       id = id + 1
       object%refcount=>new_refcount("picker_type", object%name)
       object%refcount%id = id
    end if
    
  end subroutine addref_picker_type
  
  subroutine incref_picker_type(object)
    !!< Increment the reference count of object. If there are no references
    !!< then error.
    use libsupermesh_parallel_tools, only: abort_if_in_parallel_region
    type(picker_type), intent(in), target :: object
    ...
    if (.not.associated(object%refcount)) then
       FLAbort ("Attempt to incref picker_type "//trim(object%name)//" which has no references")
    end if
  ...
  end subroutine incref_picker_type  
  
  subroutine decref_picker_type(object)
    ...
    use libsupermesh_parallel_tools, only: abort_if_in_parallel_region
    type(picker_type), intent(inout) :: object
  ...
  end subroutine decref_picker_type

  pure function has_references_picker_type(object) result (has_references)
    !!< Return true if there are any references to object
    type(picker_type), intent(in) :: object
    logical :: has_references
    
    has_references=associated(object%refcount)

  end function has_references_picker_type
