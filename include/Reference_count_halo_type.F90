  subroutine addref_halo_type(object)
    ...
    type(halo_type), intent(inout), target :: object
    integer, save :: id = 0
    ...
    if (associated(object%refcount)) then
       ...
       id = id + 1
       object%refcount=>new_refcount("halo_type", object%name)
       object%refcount%id = id
    end if
    
  end subroutine addref_halo_type
  
  subroutine incref_halo_type(object)
    !!< Increment the reference count of object. If there are no references
    !!< then error.
    use libsupermesh_parallel_tools, only: abort_if_in_parallel_region
    type(halo_type), intent(in), target :: object
...
!       FLAbort ("Attempt to incref halo_type "//trim(object%name)//" which has no references")		! ToDo
    end if
  ...
  end subroutine incref_halo_type  
  
  subroutine decref_halo_type(object)
    ...
    use libsupermesh_parallel_tools, only: abort_if_in_parallel_region
    type(halo_type), intent(inout) :: object
...
!          FLAbort("that should never happen.")					! ToDo
  ...
  end subroutine decref_halo_type

  pure function has_references_halo_type(object) result (has_references)
    !!< Return true if there are any references to object
    type(halo_type), intent(in) :: object
    logical :: has_references
    
    has_references=associated(object%refcount)

  end function has_references_halo_type
