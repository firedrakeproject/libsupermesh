  subroutine addref_csr_matrix(object)
    ...
    type(csr_matrix), intent(inout), target :: object
    integer, save :: id = 0
    ...
    if (associated(object%refcount)) then
       ...
       id = id + 1
       object%refcount=>new_refcount("csr_matrix", object%name)
       object%refcount%id = id
    end if
    
  end subroutine addref_csr_matrix
  
  subroutine incref_csr_matrix(object)
    !!< Increment the reference count of object. If there are no references
    !!< then error.
    use libsupermesh_parallel_tools, only: abort_if_in_parallel_region
    type(csr_matrix), intent(in), target :: object
  ...
  end subroutine incref_csr_matrix  
  
  subroutine decref_csr_matrix(object)
    ...
    use libsupermesh_parallel_tools, only: abort_if_in_parallel_region
    type(csr_matrix), intent(inout) :: object
  ...
  end subroutine decref_csr_matrix

  pure function has_references_csr_matrix(object) result (has_references)
    !!< Return true if there are any references to object
    type(csr_matrix), intent(in) :: object
    logical :: has_references
    
    has_references=associated(object%refcount)

  end function has_references_csr_matrix
