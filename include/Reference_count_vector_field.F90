  subroutine addref_vector_field(object)
    ...
    type(vector_field), intent(inout), target :: object
    integer, save :: id = 0
  ...
  subroutine incref_vector_field(object)
    ...
    !!< then error.
    use libsupermesh_parallel_tools, only: abort_if_in_parallel_region
    type(vector_field), intent(in), target :: object
...
!       FLAbort ("Attempt to incref vector_field "//trim(object%name)//" which has no references")	! ToDo
    end if
  ...
  subroutine decref_vector_field(object)
    ...
    use libsupermesh_parallel_tools, only: abort_if_in_parallel_region
    type(vector_field), intent(inout) :: object
...
!          FLAbort("that should never happen.")			! ToDo
  ...
  pure function has_references_vector_field(object) result (has_references)
    ...
    type(vector_field), intent(in) :: object
