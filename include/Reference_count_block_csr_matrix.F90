  subroutine addref_block_csr_matrix(object)
    ...
    type(block_csr_matrix), intent(inout), target :: object
    integer, save :: id = 0
    ...
    if (associated(object%refcount)) then
       ...
       id = id + 1
       object%refcount=>new_refcount("block_csr_matrix", object%name)
       object%refcount%id = id
    end if
    
  end subroutine addref_block_csr_matrix
  
  subroutine incref_block_csr_matrix(object)
    !!< Increment the reference count of object. If there are no references
    !!< then error.
    use libsupermesh_parallel_tools, only: abort_if_in_parallel_region
    type(block_csr_matrix), intent(in), target :: object
...
!       FLAbort ("Attempt to incref block_csr_matrix "//trim(object%name)//" which has no references")	! ToDo
    end if
  ...
  end subroutine incref_block_csr_matrix  
  
  subroutine decref_block_csr_matrix(object)
    ...
    use libsupermesh_parallel_tools, only: abort_if_in_parallel_region
    type(block_csr_matrix), intent(inout) :: object
    ...
    if (object%refcount%count<=0) then
       ...
       if (object%refcount%count<0) then
          ...
          ewrite(0,'(a, i0)') "Reference count of &
               &block_csr_matrix "//trim(object%name)//&
...
!          FLAbort("that should never happen.")			! ToDo
  ...
  end subroutine decref_block_csr_matrix

  pure function has_references_block_csr_matrix(object) result (has_references)
    !!< Return true if there are any references to object
    type(block_csr_matrix), intent(in) :: object
    logical :: has_references
    
    has_references=associated(object%refcount)

  end function has_references_block_csr_matrix
