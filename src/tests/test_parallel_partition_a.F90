...
!  end do
  ...
  call MPI_Gather(time_parallel, 1, MPI_DOUBLE_PRECISION, &
      ...
      & mpi_my_root, MPI_COMM_WORLD, mpi_my_error)
  CALL MPI_REDUCE(parallel_ele_A, local_sum, 1, MPI_INT, MPI_SUM, mpi_my_root, MPI_COMM_WORLD, mpi_my_error)
...
!    print "(i5,a,e25.17e3,a)", mpi_my_id, ": Total parallel intersection area:", sum(areas_parallel),"."
    ...
    fail = ( serial_ele_A .ne. local_sum )
    call report_test("[test_parallel_partition_a elementsA]", fail, .false., "Should give the same number of elements for mesh A")
    if (fail) then
      print "(a,i5,a,i5,a)", ": Total parallel elements for mesh A:",local_sum,&
                           & ", total serial elements for mesh A:",serial_ele_A,"."
    end if

    fail = ( serial_ele_B .ne. parallel_ele_B )
    call report_test("[test_parallel_partition_a elementsB]", fail, .false., "Should give the same number of elements for mesh B")
    if (fail) then
      print "(a,i5,a,i5,a)", ": Total parallel elements for mesh B:",parallel_ele_B,&
