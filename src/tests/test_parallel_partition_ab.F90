...
#include <finclude/petsc.h90>
  ...
  integer :: i, j, k, l, nnodes, ele_A, ele_B, ele_C, n_trisC, nprocs, &
...
! find out MY process ID, and how many processes were started.
  ...
  allocate(number_of_elements_to_receive(0:nprocs - 1), &
         & request(2 * nprocs), &
         & status(MPI_STATUS_SIZE, 2 * nprocs))
  number_of_elements_to_receive = 0
  forall(i = 1:2 * nprocs)
    status(:, i) = MPI_STATUS_IGNORE
  end forall
  serial_local_iter = 0
...
!    deallocate(map_AB)
  ...
  allocate(partition_intersection(0:nprocs - 1))
  ...
  do i = 0, nprocs - 1
    ...
    if(bboxes_intersect(bbox_a, parallel_bbox_b(:, :, i))) then
      if(i /= rank) then
        k = k + 1
        partition_intersection(i) = .true.

        call MPI_Irecv(number_of_elements_to_receive(i), 1, &
            & MPI_INTEGER, &
            & i, MPI_ANY_TAG, MPI_COMM_WORLD, &
            & request(k), ierr);  CHKERRQ(ierr)
      end if
    end if
    ...
    if(bboxes_intersect(bbox_b, parallel_bbox_a(:, :, i))) then
      if(i /= rank) then
        ...
        call MPI_Isend(l, 1, MPI_INTEGER, i, 0, &
            & MPI_COMM_WORLD, request(k), ierr);  CHKERRQ(ierr)
  ...
  call MPI_Waitall(k, request, status, ierr);  CHKERRQ(ierr)

  do i = 0, nprocs - 1
    if(partition_intersection(i)) then
  ...
  do j = 0,nprocs - 1
    ...
    if(partition_intersection(j)) then
      k = k + 1
    ...
    if(bboxes_intersect(bbox_b, parallel_bbox_a(:, :, j))) then
      k = k + 1
      call MPI_Isend(send_buffer(j)%p, &
        & size(send_buffer(j)%p), &
        & MPI_DOUBLE_PRECISION, &
        & j, 0, MPI_COMM_WORLD, request(k), ierr);  CHKERRQ(ierr)
    end if
  end do
  call MPI_Waitall(k, request, status, ierr);  CHKERRQ(ierr)
  ...
  do ele_A = 1, ele_count(positionsA)
    ...
    do j = 0, nprocs - 1
      if(j == rank) cycle
      if(.not. partition_intersection(j)) cycle
