...
subroutine test_parallel_supermesh() bind(c)
...
#include <mpif.h>
  ...
  integer :: ele_a, ele_b, ele_c, i, ierr, int_extent, loc_b, n_tris_c, &
    & nelements_a, nelements_b, nprocs, rank, real_extent
  ...
  if(rank == root) then
    call read_node("data/square_0_01.node", dim, positions_a)
    call read_ele("data/square_0_01.ele", dim, enlist_a)
    call read_node("data/triangle_0_01.node", dim, positions_b)
    call read_ele("data/triangle_0_01.ele", dim, enlist_b)
  ...
  call read_node("data/square_0_01_" // trim(nprocs_chr) // "_" // trim(rank_chr) // ".node", dim, positions_a)
  call read_ele("data/square_0_01_" // trim(nprocs_chr) // "_" // trim(rank_chr) // ".ele", dim, enlist_a)
  ...
  call read_node("data/triangle_0_01_" // trim(nprocs_chr) // "_" // trim(rank_chr) // ".node", dim, positions_b)
  call read_ele("data/triangle_0_01_" // trim(nprocs_chr) // "_" // trim(rank_chr) // ".ele", dim, enlist_b)
...
end subroutine test_parallel_supermesh
