...
#include <mpif.h>
  ...
  if(rank == 0) then
    call read_node("data/square_0_01.node", dim = 2, coords = positions_a)
    call read_ele("data/square_0_01.ele", dim = 2, enlist = enlist_a)
    call read_node("data/triangle_0_01.node", dim = 2, coords = positions_b)
  ...
  call read_node("data/square_0_01_" // trim(nprocs_chr) // "_" // trim(rank_chr) // ".node", dim = 2, coords = positions_a)
  ...
  call read_node("data/triangle_0_01_" // trim(nprocs_chr) // "_" // trim(rank_chr) // ".node", dim = 2, coords = positions_b)
