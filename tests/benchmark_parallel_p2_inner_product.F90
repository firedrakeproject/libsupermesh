...
#include <mpif.h>
  ...
  if(rank == 0) then
    ...
    print "(a)" , ""
    print "(a,f20.10)", "(MIN) Interpolation Time, parallel = ", parallel_time_interpolation_min
    print "(a,f20.10)", "(MAX) Interpolation Time, parallel = ", parallel_time_interpolation_max
    print "(a,f20.10)", "(SUM) Interpolation Time, parallel = ", parallel_time_interpolation_sum
    print "(a,f20.10)", "(AVG) Interpolation Time, parallel = ", parallel_time_interpolation_sum / nprocs
    print "(a)" , ""
...
contains
  ...
  subroutine pack_data_a(nodes_a, eles_a, data_a)
    ...
    ndata_a = (1 + 6 * size(eles_a)) * integer_extent + nnodes_p2_a * real_extent
