...
subroutine benchmark_parallel_p1_inner_product_3d() bind(c)
...
#include <mpif.h>
  ...
  call read_node(trim(basename_a) // "_" // trim(size_chr) // "_" // trim(rank_chr) // ".node", dim = 3, coords = positions_a)
  call read_ele(trim(basename_a) // "_" // trim(size_chr) // "_" // trim(rank_chr) // ".ele", dim = 3, enlist = enlist_a)
  ...
  call read_node(trim(basename_b) // "_" // trim(size_chr) // "_"// trim(rank_chr) // ".node", dim = 3, coords = positions_b)
  call read_ele(trim(basename_b) // "_" // trim(size_chr) // "_" // trim(rank_chr) // ".ele", dim = 3, enlist = enlist_b)
  ...
  call MPI_Allreduce(volume_parallel, real_buffer, 1, MPI_DOUBLE_PRECISION, MPI_SUM, MPI_COMM_WORLD, ierr);  assert(ierr == MPI_SUCCESS)
