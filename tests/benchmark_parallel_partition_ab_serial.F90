...
subroutine benchmark_parallel_partition_ab_serial() bind(c)
...
! find out MY process ID, and how many processes were started.
  ...
  if(rank == root) then
    t0 = mpi_wtime()
    positionsA = read_triangle_files("data/square_0_05"//"_"//trim(nprocs_character), dim)
  ...
  t0 = mpi_wtime()
  positionsA = read_triangle_files(trim("data/square_0_05_")//trim(nprocs_character)//"_"//trim(rank_character), dim)
  call read_halo("data/square_0_05"//"_"//trim(nprocs_character), halo, level = 2)
    ...
    if(partition_bboxes_intersect(bbox_b, parallel_bbox_a(:, :, i))) then
      if(i /= rank) then
        ...
        call MPI_Isend(number_of_elements_to_send(i), 1, MPI_INTEGER, i, rank, &
            & MPI_COMM_WORLD, request_send(sends), ierr);  assert(ierr == MPI_SUCCESS)
  ...
  do j = 0,nprocs - 1
    ...
    if( (partition_intersection_send(j)) .and. (size(send_buffer(j)%p) /=0) ) then
      ...
      call MPI_Isend(send_buffer(j)%p, &
        ...
        & MPI_DOUBLE_PRECISION, &
        & j, 0, MPI_COMM_WORLD, request_send(sends), ierr);  assert(ierr == MPI_SUCCESS)
    end if
  end do
  ...
  if(rank == root) then
    ...
    write(output_unit, "(a)") ""
    write(output_unit, "(i5,a,F19.10,a)") rank, ": Total time comm no of elements  : ", sum(comms_A_parallel)," ."
    write(output_unit, "(i5,a,F19.10,a)") rank, ": Max time comm no of elements    : ", maxval(comms_A_parallel)," ."
    write(output_unit, "(a)") ""
    write(output_unit, "(i5,a,F19.10,a)") rank, ": Total time comm elements        : ", sum(comms_B_parallel)," ."
    write(output_unit, "(i5,a,F19.10,a)") rank, ": Max time comm elements          : ", maxval(comms_B_parallel)," ."
    write(output_unit, "(a)") ""
